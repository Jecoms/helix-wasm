...
impl Grammar {
    ...
    ///
    /// Not available on wasm32: there is no dynamic linking there, grammars
    /// must be linked statically and obtained from their `tree_sitter_*`
    /// symbols (declared as `extern "C" fn() -> Grammar`, which is
    /// ABI-compatible since `Grammar` is `repr(transparent)` over a pointer).
    #[cfg(not(target_arch = "wasm32"))]
...
/// Errors from loading a grammar out of a shared library; on wasm32 grammars
/// are statically linked instead and this error cannot occur.
#[cfg(not(target_arch = "wasm32"))]
