...
// The statically compiled tree-sitter C runtime calls libc's allocator; on
// wasm32-unknown-unknown there is no libc, so provide one backed by Rust's.
#[cfg(target_arch = "wasm32")]
mod wasm_alloc;

use std::ops;

pub use grammar::{Grammar, IncompatibleGrammarError, ABI_VERSION, MIN_COMPATIBLE_ABI_VERSION};
pub use node::Node;
