...
// instance down with it. These specs drive the two classes of trigger the
// issue names and assert the editor is still alive afterwards: the job's own
// message reached the statusline, and the next keystroke still lands.
//
...
test("a pure job (:tree-sitter-scopes) does its work rather than dying", async ({
  ...
  // The popup renders the scope list as JSON; an empty scratch buffer has
  // none, so the empty list is what draws.
