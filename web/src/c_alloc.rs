...
//! global allocator.
//!
//! The statically linked tree-sitter runtime (and grammar scanners) call
//! `malloc`, `calloc`, `realloc` and `free`. There is no libc on
//! wasm32-unknown-unknown, so these definitions are the only ones in the
//! final link (the rest of the C runtime surface lives in
//! `sysroot/shims.c`). Rust's allocator needs the `Layout` back at
//! deallocation time, which C callers don't provide, so each allocation
//! stores its size in a header directly before the pointer handed to C.
