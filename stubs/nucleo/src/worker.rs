...
/// The dedicated threadpool match runs execute on. On wasm32-unknown-unknown
/// no custom rayon pool can exist (thread spawning is unsupported), so there
/// `spawn` runs the job inline on the calling thread and the parallel
/// iterators inside it execute through rayon's documented single-threaded
...
impl<T: Sync + Send + 'static> Worker<T> {
    ...
    pub(crate) fn new(
        ...
        // wasm32 runs matches inline on the single main thread.
        #[cfg(target_arch = "wasm32")]
        let worker_threads = {
            let _ = worker_threads;
            1
        };
        let pool = Pool::new(worker_threads);
