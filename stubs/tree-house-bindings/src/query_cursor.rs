...
extern "C" {
    ...
    /// will be executed.
    ///
    /// Returns `false` when the given range is invalid (`start_byte` past
    /// `end_byte`); see `ts_query_cursor_set_byte_range` in
    /// `vendor/include/tree_sitter/api.h`. Upstream declares this as
    /// returning nothing, which is an ABI mismatch — harmless where the
    /// calling convention ignores the difference, but a `signature_mismatch`
    /// trap on wasm32 the moment a query cursor runs.
