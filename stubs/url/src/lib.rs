...
#[cfg(any(
    ...
    target_os = "redox",
    target_os = "wasi",
    target_os = "hermit", target_arch = "wasm32"
))]
...
mod wasm_os_str_ext {
    ...
    impl OsStrExt for std::ffi::OsStr {
        fn as_bytes(&self) -> &[u8] {
            self.to_str().expect("non-utf8 path on wasm").as_bytes()
        }
    }
}
...
#[cfg(all(
    feature = "std",
    any(unix, target_os = "redox", target_os = "wasi", target_os = "hermit", target_arch = "wasm32")
))]
...
fn file_url_segments_to_pathbuf(
    ...
    #[cfg(not(any(target_os = "wasi", all(target_arch = "wasm32", not(target_os = "wasi")))))]
